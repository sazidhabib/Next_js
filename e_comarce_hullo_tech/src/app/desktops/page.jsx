...
export default async function DesktopsPage() {
  ...
  try {
    if (Product) {
      const dbProducts = await Product.findAll({ where: { category: "desktops" } });
      categoryProducts = dbProducts.map((p) => p.toJSON());
    }
  } catch (error) {
    console.error("DB Fetch failed for desktops, using mock fallback:", error);
  }

  if (categoryProducts.length === 0) {
    categoryProducts = mockProducts.filter((p) => p.category === "desktops");
  }
    ...
    <main className="min-h-screen bg-star-light-gray pt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <ul className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <li className="flex items-center gap-2">
            ...
            <span className="text-gray-900 font-semibold">Desktops</span>
          </li>
        </ul>
      </div>
      ...
      <div className="bg-white border-b border-star-gray py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Desktops</h1>
          <p className="text-gray-600">
            Explore our collection of desktop PCs, gaming PCs, and workstations.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <SubCategoryHeader customCategory="/desktops" />
        <div className="bg-white rounded-lg border border-star-gray p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <select className="px-4 py-2 border border-star-gray rounded text-sm focus:outline-none focus:border-star-blue">
              ...
              <option>MiniTech</option>
              <option>Star PC</option>
            </select>
          </div>

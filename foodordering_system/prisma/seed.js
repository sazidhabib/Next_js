...
async function main() {
  ...
  try {
    ...
    console.log('Note: DB clean skipped or table empty:', err.message);
  }
  ...
  const staffOperator = await prisma.user.create({
    data: {
  ...
  // Assign roles
  await prisma.userRestaurantRole.createMany({
    data: [
  ...
  const pizzaSizeGroup = await prisma.optionGroup.create({
    data: {
      restaurantId: restaurant.id,
      ...
      items: {
        create: [
          { name: 'Medium 12" (6 Slices)', price: 0.0, isDefault: true, displayOrder: 1 },
          { name: 'Large 14" (8 Slices)', price: 4.5, isDefault: false, displayOrder: 2 },
          { name: 'Family Grand 18" (12 Slices)', price: 9.5, isDefault: false, displayOrder: 3 },
        ],
      },
    },
  });
  ...
  const crustGroup = await prisma.optionGroup.create({
    data: {
      ...
      items: {
        create: [
          { name: 'Traditional Neapolitan Stone Crust', price: 0.0, isDefault: true, displayOrder: 1 },
          { name: 'Thin & Crispy Roman Style', price: 0.0, isDefault: false, displayOrder: 2 },
          { name: 'Stuffed Garlic & Mozzarella Crust', price: 3.0, isDefault: false, displayOrder: 3 },
          { name: 'Gluten-Free Cauliflower Crust', price: 4.0, isDefault: false, displayOrder: 4 },
        ],
      },
    },
  });

  const extraToppingsGroup = await prisma.optionGroup.create({
    data: {
      restaurantId: restaurant.id,
      ...
      items: {
        create: [
          { name: 'Extra Buffalo Mozzarella D.O.P', price: 2.5, displayOrder: 1 },
          { name: 'Artisan Spicy Pepperoni', price: 2.75, displayOrder: 2 },
          { name: 'Prosciutto di Parma Riserva', price: 3.5, displayOrder: 3 },
          { name: 'Wild Truffle Cremini Mushrooms', price: 2.25, displayOrder: 4 },
          { name: 'Spicy Calabrian Chili Crunch', price: 1.25, displayOrder: 5 },
          { name: 'Fresh Sweet Basil & EVOO Drizzle', price: 0.75, displayOrder: 6 },
        ],
      },
    },
  });

  const burgerDonenessGroup = await prisma.optionGroup.create({
    data: {
      restaurantId: restaurant.id,
      ...
      items: {
        create: [
          { name: 'Medium Pink (Juicy)', price: 0.0, isDefault: true, displayOrder: 1 },
          { name: 'Medium Well', price: 0.0, isDefault: false, displayOrder: 2 },
          { name: 'Well Done (Charred)', price: 0.0, isDefault: false, displayOrder: 3 },
        ],
      },
    },
  });

  const burgerAddonsGroup = await prisma.optionGroup.create({
    data: {
      restaurantId: restaurant.id,
      ...
      items: {
        create: [
          { name: 'Thick Applewood Smoked Bacon', price: 2.5, displayOrder: 1 },
          { name: 'Aged White Truffle Cheddar', price: 1.75, displayOrder: 2 },
          { name: 'Crispy Sunny-Side Organic Fried Egg', price: 1.5, displayOrder: 3 },
          { name: 'Double Meat Patty (Extra 1/3 lb Angus)', price: 4.5, displayOrder: 4 },
        ],
      },
    },
  });
  ...
  for (const item of catPizzas.items) {
  ...
  for (const item of catBurgers.items) {
    await prisma.menuItemOptionGroup.createMany({
      data: [
  ...
  const catPastas = await prisma.category.create({
    data: {
      restaurantId: restaurant.id,
  ...
  const catDolci = await prisma.category.create({
    data: {
      restaurantId: restaurant.id,

...
export default function HomeTypeTester({ fonts = [] }) {
  ...
  return (
    <div className="bg-[#12141f] border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-2xl">
      ...
      <div className="flex items-center gap-2 mb-4 sm:mb-6">
        ...
        <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">Interactive Type Tester</h3>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-start">
        {/* Left Side: Main Type Tester Board */}
        <div className="lg:col-span-8 bg-[#161824] border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 flex flex-col justify-between min-h-[300px] sm:min-h-[460px]">
          <div>
            {/* Controls Bar */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 pb-3 sm:pb-4 border-b border-white/10 text-xs text-gray-300">
              <div className="flex items-center w-full sm:w-auto">
                <select
                  value={text}
                  onChange={(e) => setText(e.target.value)}
                  className="bg-[#1e2130] text-gray-200 border border-white/10 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1.5 focus:outline-none focus:border-[#00e599] text-[10px] sm:text-xs cursor-pointer w-full sm:w-auto"
                >
          ...
          <div className="pt-3 sm:pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-1 bg-[#1d2030] border border-white/10 p-1 rounded-lg sm:rounded-xl">
              <button
                type="button"
                onClick={() => setTextAlign("left")}
                className={`p-1 sm:p-1.5 rounded-md sm:rounded-lg text-xs font-semibold ${textAlign === "left" ? "bg-[#00e599] text-gray-950 font-bold" : "text-gray-400 hover:text-white"}`}
              >
                ≡
              </button>
              <button
                ...
                className={`p-1 sm:p-1.5 rounded-md sm:rounded-lg text-xs font-semibold ${textAlign === "center" ? "bg-[#00e599] text-gray-950 font-bold" : "text-gray-400 hover:text-white"}`}
              >
                ≡
              </button>
              <button
                ...
                className={`p-1 sm:p-1.5 rounded-md sm:rounded-lg text-xs font-semibold ${textAlign === "right" ? "bg-[#00e599] text-gray-950 font-bold" : "text-gray-400 hover:text-white"}`}
              >
                ≡
              </button>
              <button
        ...
        <div className="lg:col-span-4 bg-[#161824] border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5 min-h-[300px] sm:min-h-[460px] flex flex-col">
          <div className="mb-3">
            <input
              type="text"
              value={searchQuery}
              ...
              className="w-full bg-[#1e2130] border border-white/10 text-white text-xs px-3 sm:px-3.5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl focus:outline-none focus:border-[#00e599] placeholder-gray-500"
            />
          </div>

          <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-3 border-b border-white/5 pb-3">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.value}
                type="button"
          ...
          <div className="flex-1 overflow-y-auto max-h-[240px] sm:max-h-[340px] space-y-2 pr-1 custom-scrollbar">
            {filteredFonts.length === 0 ? (
              <p className="text-xs text-gray-500 text-center py-8">কোনো ফন্ট পাওয়া যায়নি।</p>
            ) : (
              filteredFonts.map((f) => (
                <div
                  key={f.id}

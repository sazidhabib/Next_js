...
export default function FrameDetailsClient({ initialFrame }) {
    const { id } = useParams();
    const router = useRouter();

    // Data State
    const [frame, setFrame] = useState(initialFrame || null);
    const [relatedFrames, setRelatedFrames] = useState([]);
    const [loading, setLoading] = useState(!initialFrame);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // Fetch current frame if not provided as initialFrame
                if (!frame) {

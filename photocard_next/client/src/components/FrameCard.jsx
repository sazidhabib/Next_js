    ...
    return (
        <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden border border-gray-100 group">
            <div className="relative aspect-[4/3] overflow-hidden bg-gray-50 p-4">
                ...
                <Link href={`/frame/${id}`} className="absolute inset-0 flex items-center justify-center transition-transform duration-500 group-hover:scale-105">
                    {image ? (
            ...
            <div className="p-4 text-center">
                <Link href={`/frame/${id}`} className="block">
                    <h3 className="font-bold text-gray-800 text-lg mb-1 hover:text-primary transition-colors">{title}</h3>
                </Link>
                ...
                <Link
                    href={`/frame/${id}`}
                    className="w-full mt-2 py-2 px-4 rounded-lg bg-white border border-blue-600 text-blue-700 font-medium hover:bg-blue-500 hover:text-white transition-colors flex items-center justify-center gap-2"
                >
                    ফ্রেম তৈরি করুন
                    <ArrowRight size={16} />
                </Link>

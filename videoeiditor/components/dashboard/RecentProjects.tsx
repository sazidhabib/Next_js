      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {filteredProjects.map((project) => (
            <div key={project.id} className="bento-card rounded-xl overflow-hidden flex flex-col group">
              <div className="relative h-48 bg-surface-container-lowest">
                ...
                {project.status && (
                  <div className="absolute top-2 right-2 px-2 py-0.5 bg-brand-primary bg-[#3B82F6] text-white text-[10px] font-bold rounded uppercase">
                    {project.status}
                  </div>
                )}
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                  <button 
                    ...
                    className="bg-primary text-on-primary w-10 h-10 rounded-full flex items-center justify-center shadow-xl transform scale-75 group-hover:scale-100 transition-transform hover:scale-105 active:scale-95"
                    title="Open in Editor"
                  >
                    <span className="material-symbols-outlined block">edit</span>
                  </button>
                  <button className="bg-surface text-on-surface w-10 h-10 rounded-full flex items-center justify-center shadow-xl transform scale-75 group-hover:scale-100 transition-transform hover:bg-surface-container-highest active:scale-95">
              ...
              <div className="p-4">
                <h4 className="font-body-md text-body-md font-bold mb-1 truncate group-hover:text-primary transition-colors cursor-pointer" onClick={() => handleEditProject(project.id)}>
                  {project.name}
                </h4>
        ...
        <div className="w-full bg-surface-container border border-outline-variant rounded-xl overflow-hidden divide-y divide-outline-variant">
          {filteredProjects.map((project) => (
            <div key={project.id} className="flex items-center justify-between p-4 hover:bg-surface-container-high transition-colors group">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="font-body-md text-body-md font-bold truncate group-hover:text-primary cursor-pointer" onClick={() => handleEditProject(project.id)}>
                    {project.name}
                  </h4>
              ...
              <div className="flex items-center gap-6 text-xs text-on-surface-variant">
                <span className="font-mono-data bg-black/40 px-2 py-0.5 rounded">{project.duration}</span>
                <span>{project.size}</span>
                {project.status && (
                  <span className="bg-primary/20 border border-primary/30 text-primary text-[9px] px-1.5 py-0.5 rounded uppercase font-bold">
                    {project.status}
                  </span>
                )}
                <div className="flex gap-2">
                  <button 
                    onClick={() => handleEditProject(project.id)}
                    className="p-1.5 hover:bg-surface-container-highest text-primary rounded transition-all active:scale-95"
                  >
                    <span className="material-symbols-outlined text-lg block">edit</span>
                  </button>
                  <button className="p-1.5 hover:bg-surface-container-highest rounded transition-all active:scale-95">
